"use client";

import Image from "next/image";
import SubredditStats from "./SubredditStats";

const DISCORD_LINK = "https://discord.gg/asianmasc";

const heroPhotos = [
  { src: "/community-1.webp", alt: "Community member — clean style" },
  { src: "/community-3.webp", alt: "Community member — bold style" },
  { src: "/community-2.webp", alt: "Community member — natural style" },
];

export default function Hero() {
  return (
    <section
      aria-labelledby="hero-heading"
      className="relative min-h-screen flex items-center justify-center overflow-hidden"
    >
      {/* Background gradient - decorative */}
      <div
        className="absolute inset-0 bg-gradient-to-br from-dark-900 via-dark-800 to-dark-900"
        aria-hidden="true"
      />

      {/* Accent glow effects - decorative */}
      <div
        className="absolute top-1/4 left-1/4 w-96 h-96 bg-accent-red/10 rounded-full blur-3xl"
        aria-hidden="true"
      />
      <div
        className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-accent-gold/10 rounded-full blur-3xl"
        aria-hidden="true"
      />

      {/* Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full">
        <div className="grid lg:grid-cols-[1fr,auto] gap-12 items-center">
          {/* Left — text content */}
          <div className="text-center lg:text-left">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-dark-700/80 border border-dark-500 mb-8">
              <span
                className="w-2 h-2 bg-green-500 rounded-full animate-pulse"
                aria-hidden="true"
              />
              <span className="text-sm text-gray-300">
                <SubredditStats /> in the r/asianmasculinity subreddit
              </span>
            </div>

            {/* Main headline */}
            <h1
              id="hero-heading"
              className="text-5xl md:text-7xl font-bold mb-6 leading-tight"
            >
              A Community Built for{" "}
              <span className="gradient-text-shimmer">Your Growth</span>
            </h1>

            {/* Subheadline */}
            <p className="text-xl md:text-2xl text-gray-300 mb-4 max-w-3xl">
              The largest community of Asian men leveling up together.
            </p>
            <p className="text-lg text-gray-400 mb-10 max-w-2xl">
              Fitness. Dating. Career. Business. Culture.
              <br />
              <span className="text-white font-medium">Join the movement.</span>
            </p>

            {/* CTA Buttons */}
            <div
              className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start"
              role="group"
              aria-label="Call to action"
            >
              <a
                href={DISCORD_LINK}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Join the AsianMasc Discord community (opens in new tab)"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-accent-red to-accent-red-dark hover:from-accent-red-dark hover:to-accent-red text-white font-semibold rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-accent-red/25"
              >
                <svg
                  className="w-6 h-6"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  aria-hidden="true"
                >
                  <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
                </svg>
                Join the Community
              </a>
              <a
                href="#pillars"
                aria-label="Learn more about our community pillars"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-dark-700 hover:bg-dark-600 text-white font-semibold rounded-xl border border-dark-500 transition-all duration-300"
              >
                Learn More
                <svg
                  className="w-5 h-5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M19 9l-7 7-7-7"
                  />
                </svg>
              </a>
            </div>
          </div>

          {/* Right — photo strip (desktop only) */}
          <div
            className="hidden lg:flex flex-col gap-4 pr-4"
            aria-hidden="true"
          >
            {heroPhotos.map((photo, i) => (
              <div
                key={photo.src}
                className={`relative w-44 h-56 rounded-2xl overflow-hidden border border-dark-500/50 shadow-2xl transition-transform duration-700 ${
                  i === 0
                    ? "translate-x-4 rotate-2"
                    : i === 1
                    ? "-translate-x-2 -rotate-1"
                    : "translate-x-6 rotate-3"
                }`}
              >
                <Image
                  src={photo.src}
                  alt={photo.alt}
                  fill
                  className="object-cover object-top"
                  sizes="176px"
                />
                {/* Subtle gold border glow */}
                <div className="absolute inset-0 rounded-2xl ring-1 ring-accent-gold/20" />
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Scroll indicator - decorative */}
      <div
        className="absolute bottom-8 left-0 right-0 flex justify-center animate-bounce"
        aria-hidden="true"
      >
        <svg
          className="w-6 h-6 text-gray-500"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M19 14l-7 7m0 0l-7-7m7 7V3"
          />
        </svg>
      </div>
    </section>
  );
}
